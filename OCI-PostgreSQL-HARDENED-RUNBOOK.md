...
- `discover_oci_dbsystems.sh` → discover OCI PostgreSQL DBSystem OCIDs for bridge filtering
...
```bash
sudo cp /Users/shadab/Downloads/{generate_configs.sh,deploy_stack.sh,start_stack.sh,stop_stack.sh,check_grafana_bindings.sh,discover_oci_dbsystems.sh,destroy_stack.sh,rotate_certs.sh,smoke_test.sh} /opt/observability-stack/
sudo chmod +x /opt/observability-stack/{generate_configs.sh,deploy_stack.sh,start_stack.sh,stop_stack.sh,check_grafana_bindings.sh,discover_oci_dbsystems.sh,destroy_stack.sh,rotate_certs.sh,smoke_test.sh}
```
...
> `OCI_CONFIG_PROFILE` is sanitized by generator (strips accidental trailing `:` or `/` and brackets) to avoid profile issues like `DEFAULT/:`.

### Built-in OCI collector (OCI -> Prometheus)

If you want OCI metrics visible through Grafana Prometheus datasource, you can use the built-in collector service generated by this repo.
...
- If `OCI_PROM_SCRAPE_TARGETS` is empty, Prometheus defaults to built-in target `oci_metrics_collector:${OCI_PROM_PORT}`.
- `OCI_PROM_DBSYSTEM_IDS` accepts comma-separated multiple DBSystem OCIDs.
- `OCI_PROM_AUTO_DISCOVER_DBSYSTEMS=true` enables runtime discovery of seen resource IDs.
- `OCI_PROM_DISCOVER_DBSYSTEMS_ON_DEPLOY=true` triggers pre-generation discovery by running `discover_oci_dbsystems.sh` and merging discovered IDs.

Manual discovery:

```bash
bash /opt/observability-stack/discover_oci_dbsystems.sh
```

### Optional: append additional scrape jobs

If you also need custom non-collector scrape jobs, append via:
...
When enabled, unified dashboard adds **All Metrics via Prometheus Bridge** panel with variable filters for region/compartment/dbsystem/metric regex.
