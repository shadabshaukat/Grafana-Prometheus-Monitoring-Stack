...
log "[1/4] Verifying Prometheus datasource exists"
...
log "[2/4] Fetching dashboard JSON"
...
| [
    ...
    ($metric_panels[] as $p
      | ($p.targets // [])[]?
      | select(ds_uid(.datasource) != $prom_uid or ds_type(.datasource) != "prometheus")
      | "Panel \($p.title): target \(.refId // "?") missing explicit Prometheus datasource uid=\($prom_uid), type=prometheus")
  ]
| map(select(. != null and . != ""))
| .[]
JQ
